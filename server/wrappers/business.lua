...
function GeneratebusinessAccount(acc, sc, bid)
    ...
    local bankAccount = MySQL.single.await('SELECT * FROM bank_accounts WHERE account_number = ? AND sort_code = ? AND businessid = ?', {
        self.accountNumber,
        self.sortCode,
        self.bid
    })
    ...
    if bankAccount then
        self.account_id = bankAccount.record_id
        self.balance = bankAccount.amount
        self.account_type = "business"
        self.account_for = bankAccount.business
...
local function createbusinessAccount(accttype, bid, startingBalance)
    ...
    local checkExists = MySQL.single.await('SELECT COUNT(1) FROM `bank_accounts` WHERE `business` = ? AND `businessid` = ?', {
        accttype,
        bid
    })
